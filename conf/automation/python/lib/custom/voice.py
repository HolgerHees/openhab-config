...
class VoiceAssistentHelper:
    ...
    #    Registry.getItem(VoiceAssistentHelper._getMessageDeviceItemByLocation(location)).sendCommand(message)
    ...
    def sendTTS(message, header = None, location = None, priority = NotificationHelper.PRIORITY_INFO, effects = 0 ):
        ...
        if not is_announcement and effects & VoiceAssistentHelper.EFFECT_WISPER:
        ...
        if location is None:
            ...
            if priority == NotificationHelper.PRIORITY_ALERT:
                Registry.getItem(VoiceAssistentHelper._getTTSDeviceItemByLocation(location)).sendCommand("<speak>Alarm <audio src=\"soundbank://soundlibrary/scifi/amzn_sfx_scifi_alarm_06\"/> {}</speak>".format(message))
            else:
                Registry.getItem(VoiceAssistentHelper._getTTSDeviceItemByLocation(location)).sendCommand("<speak>{}</speak>".format(message))

    @staticmethod
    def sendMessage(message, interactive):
        if interactive:
