        ...
        if automatic_rollershutter_state != SunProtectionHelper.STATE_ROLLERSHUTTER_UP:
            ...
            if now.hour < 12 and now.date() == _up_time.date():
                if now >= _up_time:
                    ...
                    _up_time = Registry.getItemState("pOutdoor_Astro_Dawn_Time")
                    if now >= _up_time:
            ...
            _down_time = Registry.getItemState("pOutdoor_Astro_Dusk_Time")
            ...
            if now >= _down_time:
                ...
                _down_time = Registry.getItemState("pOutdoor_Astro_Sunset_Time")
                if now >= _down_time:
                    Registry.getItem("pOther_Automatic_State_Rollershutter").postUpdate(SunProtectionHelper.STATE_ROLLERSHUTTER_DOWN)
