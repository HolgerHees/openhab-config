...
class ValuesDependingOnBrightnessRule:
    ...
    STATE_ROLLERSHUTTER_MAYBE_DOWN = 1
    STATE_ROLLERSHUTTER_MAYBE_UP = 2
    STATE_ROLLERSHUTTER_UP = 3
...
#        self.triggers = [CronTrigger("*/15 * * * * ?")]
    ...
    def execute(self, module, input):
        ...
        if getItemState("pOther_Automatic_State_Rollershutter") == ValuesDependingOnBrightnessRule.STATE_ROLLERSHUTTER_DOWN:
            ...
            if _upTime > _lastDownTime:
                if lightLevel > 0 and nowInMillies > _upTime:
                    postUpdate("pOther_Automatic_State_Rollershutter", ValuesDependingOnBrightnessRule.STATE_ROLLERSHUTTER_MAYBE_UP)
                else:
                    ...
                    if nowInMillies >= _upTime:
                        postUpdate("pOther_Automatic_State_Rollershutter", ValuesDependingOnBrightnessRule.STATE_ROLLERSHUTTER_UP)
            ...
            if lightLevel <= 0 and nowInMillies >= _downTime:
                postUpdate("pOther_Automatic_State_Rollershutter", ValuesDependingOnBrightnessRule.STATE_ROLLERSHUTTER_DOWN)
            else:
                ...
                if nowInMillies >= _downTime:
                    postUpdate("_downTime", ValuesDependingOnBrightnessRule.STATE_ROLLERSHUTTER_DOWN)
