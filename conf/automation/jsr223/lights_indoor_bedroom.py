...
class LightsIndoorBedroomLeftControlRule:
    def __init__(self):
        self.triggers = [
            ItemStateUpdateTrigger("Light_FF_Bedroom_Left", state="ON")
        ]

    def execute(self, module, input):
        if getItemState("Light_FF_Bedroom_Left_Hue_Brightness").intValue() == 0:
            sendCommand("Light_FF_Bedroom_Left_Hue_Brightness",100)
        else:
            sendCommand("Light_FF_Bedroom_Left_Hue_Brightness",0)
            
@rule("lights_indoor_bedroom_right_control.py")
class LightsIndoorBedroomRightControlRule:
    def __init__(self):
        self.triggers = [
            ItemStateUpdateTrigger("Light_FF_Bedroom_Right", state="ON")
        ]

    def execute(self, module, input):
        if getItemState("Light_FF_Bedroom_Right_Hue_Brightness").intValue() == 0:
            sendCommand("Light_FF_Bedroom_Right_Hue_Brightness",100)
        else:
            sendCommand("Light_FF_Bedroom_Right_Hue_Brightness",0)
