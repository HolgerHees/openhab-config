...
class ScenesWateringControl():
    ...
    def findStep(self):
        ...
        if remaining <= 0:
            ...
            if nextIndex == -1:
                sendCommand("pOutdoor_Watering_Logic_Program_Start", ScenesWateringHelper.STATE_OFF)
            else:
    ...
    def triggerProgramTimer(self):
        ...
        if active_programm not in [ScenesWateringHelper.STATE_START_MORNING, ScenesWateringHelper.STATE_START_EVENING]:
            self.log.error("Invalid call of triggerProgramTimer")
            return

        sendCommand("pOutdoor_Watering_Logic_Program_Start", ScenesWateringHelper.STATE_START_NOW)
    ...
    def initProgramTimer(self, input = None):
        ...
        if active_programm == ScenesWateringHelper.STATE_START_MORNING:
            #next_start = now.plusSeconds(total_duration * 60 + 10)
            next_start = now.withHour(8).withMinute(0).withSecond(0)
        elif active_programm == ScenesWateringHelper.STATE_START_EVENING:
    ...
    def execute(self, module, input):
        ...
        if input['event'].getItemName() == "pOutdoor_Watering_Logic_Program_Start":
            if input["command"].intValue() == ScenesWateringHelper.STATE_START_NOW:
                self.log.info("callbackProgress")
                self.callbackProgress()
                return
            else:
                self.disableAllCircuits()
                if input["command"].intValue() == ScenesWateringHelper.STATE_OFF:
                    return
